...
vim.api.nvim_create_autocmd('BufReadPost', {
  pattern = '*.cpp',
  callback = function()
    local line = vim.fn.search('// Agar pura solution bana hai tabhi code krna bhai', 'n')
    if line ~= 0 then
...
vim.api.nvim_create_autocmd('FileType', {
  ...
  callback = function()
    ...
    vim.opt_local.shiftwidth = 4 -- indent by 4 visually
...
require('lazy').setup({
  ...
  { -- Autocompletion
    ...
    opts = {
      enabled = function()
        return vim.bo.filetype ~= 'cpp' and vim.bo.filetype ~= 'c'
      end,
      keymap = {
